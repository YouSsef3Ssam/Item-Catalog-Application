...
def showCatalog():
    ...
    if 'username' not in login_session:  # make sure user has logined
        return render_template('catalogforanonymous.html',
                               categories=categories,
                               items=items)
    else:  # if user logined, able to access create a new item
        return render_template('catalogforauthorize.html',
                               categories=categories,
                               items=items)
...
@app.route('/catalog/<int:categories_id>/item/<int:items_id>')
def showItem(categories_id, items_id):
    ...
    if 'username' not in login_session or \
        ...
        return render_template('itemfornonymous.html', categories=categories,
                               items=items)
    else:  # if user is the creator, able to access update and delete the item
        return render_template('itemforauthorize.html',
                               categories=categories, items=items)
...
if __name__ == '__main__':
    ...
    app.debug = True
    app.run(host='35.178.167.223', port=80)
